...
class _AddNewAddressScreenState extends State<AddNewAddressScreen> {
  ...
  late double deliveryFees ;
  List<SupZone> supZones = [];
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<ZoneProvider>(context, listen: false).fetchSupZoneData();
    });
  ...
  Widget build(BuildContext context) {
    ...
    final supZones = zoneProvider.supZones;
                        ...
                        child: Center(
                          ...
                          child: Column(children: [
                            Align(
                                ...
                                child: Text(
                                    getTranslated("add_delivery_info", context)!
                                        .toTitleCase(),
                                ...
                                children: [
                                  ...
                                  DropdownButtonFormField<String>(
                                    ...
                                    hint: Text(getTranslated(
                                        'select_delivery_region', context)!),
                                    onChanged: (newValue) {
                                      setState(() {
                                        _selectedDeliveryOption = newValue;
                                        deliveryFees = supZones
                                            .firstWhere(
                                                (zone) => zone.city == newValue)
                                            .deliveryFees;
                                        print("delivery: $deliveryFees");
                                      });
                                    },
                                    items: supZones
  ...
  void _saveAddress(LocationProvider locationProvider) {
    ...
    if (addressFormKey.currentState != null &&
      ...
      // Check if the user has selected an address type
      if (locationProvider.selectAddressIndex < 0 ||
          locationProvider.selectAddressIndex >=
              locationProvider.getAllAddressType.length) {
        showCustomSnackBarHelper(
            getTranslated("Please select a valid address type", context));
        return;
      }
      if (_selectedDeliveryOption == null) {
        showCustomSnackBarHelper(
            getTranslated("Please select a delivery region", context));
        return;
      }
      ...
      if (_streetNumberController.text.isEmpty) {
        showCustomSnackBarHelper(
            getTranslated("please enter street number", context));
        return;
      }
      if (_florNumberController.text.isEmpty) {
        showCustomSnackBarHelper(
            getTranslated("please_enter_floor_number", context));
        return;
      }
      if (_houseNumberController.text.isEmpty) {
        showCustomSnackBarHelper(
            getTranslated("please_enter_house_number", context));
        return;
      }
  ...
  void location() {
    Navigator.of(context).push(MaterialPageRoute(
        builder: (context) => CheckoutScreen(
              ...
              selected: _selectedDeliveryOption,
              amount: null,
              cartList: [],
              couponCode: "",
              fromCart: false,
    ...
    print("delivery fee: $deliveryFees ");
  }
}
