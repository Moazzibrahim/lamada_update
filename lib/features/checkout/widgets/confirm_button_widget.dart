            ...
            onTap: () {
              ...
              if (!takeAway &&
                ...
                if (checkoutProvider.selectedPaymentMethod != null ||
                    checkoutProvider.selectedOfflineValue != null) {
                  bool isAvailable = true;
                  DateTime scheduleStartDate = DateTime.now();
                  DateTime scheduleEndDate = DateTime.now();

                  if (checkoutProvider.timeSlots == null ||
                      checkoutProvider.timeSlots!.isEmpty) {
                    isAvailable = false;
                  } else {
                    DateTime date = checkoutProvider.selectDateSlot == 0
                        ? DateTime.now()
                        : DateTime.now().add(const Duration(days: 1));
                    DateTime startTime = checkoutProvider
                        .timeSlots![checkoutProvider.selectTimeSlot].startTime!;
                    DateTime endTime = checkoutProvider
                        .timeSlots![checkoutProvider.selectTimeSlot].endTime!;
                    ...
                    for (CartModel? cart in cartList) {
                      if (!DateConverterHelper.isAvailable(
                  ...
                  } else if (checkoutProvider.partialAmount != null &&
                      (checkoutProvider.selectedPaymentMethod == null
                          ? (checkoutProvider.selectedOfflineValue == null)
                          : checkoutProvider.selectedPaymentMethod == null)) {
                    ResponsiveHelper.showDialogOrBottomSheet(
                        context,
                        PaymentMethodBottomSheetWidget(
                            totalPrice: orderAmount + (deliveryCharge ?? 0)));
                  }
                  ...
                  // }
                    ...
                    PlaceOrderBody placeOrderBody = PlaceOrderBody(
                      ...
                      deliveryDate:
                          DateFormat('yyyy-MM-dd').format(scheduleStartDate),
                      paymentInfo: checkoutProvider.selectedOfflineValue != null
                          ? OfflinePaymentInfo(
                              methodFields: CheckOutHelper.getOfflineMethodJson(
                                  checkoutProvider
                                      .selectedOfflineMethod?.methodFields),
                              methodInformation:
                                  checkoutProvider.selectedOfflineValue,
                              paymentName: checkoutProvider
                                  .selectedOfflineMethod?.methodName,
                              paymentNote: checkoutProvider
                                  .selectedOfflineMethod?.paymentNote,
                            )
                          : null,
                      deliveryTime: (checkoutProvider.selectTimeSlot == 0 &&
                              checkoutProvider.selectDateSlot == 0)
                          ? 'now'
                          : DateFormat('HH:mm').format(scheduleStartDate),
                      isPartial:
                          checkoutProvider.partialAmount == null ? '0' : '1',
                    ...
                    if (placeOrderBody.paymentMethod == 'wallet_payment' ||
                      ...
                      String selectedUrl =
                          '${AppConstants.baseUrl}/payment-mobile?token=$tokenUrl&&payment_method=${checkoutProvider.selectedPaymentMethod?.getWay}&&payment_platform=${kIsWeb ? 'web' : 'app'}&&is_partial=${checkoutProvider.partialAmount == null ? '0' : '1'}';
